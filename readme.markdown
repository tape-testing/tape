...
Assert that the function call `fn()` throws an exception. `expected`, if present, must be either a `String`, `RegExp` or `Function`. The `String` matches `err.message`, the `RegExp` matches the string representation of the exception, as generated by `err.toString()`. The `Function` is the exception thrown (e.g. `Error`). `msg` is an optional description of the assertion.
